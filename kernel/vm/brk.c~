...
do_brk(void *addr, void **ret)
{
	...
	if(addr<curproc->p_start_brk||addr>=(void *)USER_MEM_HIGH)
	{
        ...
        uint32_t curpbrknum=ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_brk));/*remember to align up*/
	uint32_t newpbrknum=ADDR_TO_PN(PAGE_ALIGN_UP(addr));
	vmarea_t *temp_vmarea=vmmap_lookup(curproc->p_vmmap,curpbrknum-1);

        if(newpbrknum==curpbrknum)
        {
                temp_vmarea->vma_end=newpbrknum;
                curproc->p_brk=addr;
	        *ret = addr;
                return 0;       
        }
	if(newpbrknum>curpbrknum)
	{
		if(vmmap_is_range_empty(curproc->p_vmmap,curpbrknum,newpbrknum-curpbrknum)==0)
		{
			return -ENOMEM;
		}
		else
