...
sched_wakeup_on(ktqueue_t *q)
{
        ...
        kthread_t *temp_thread=ktqueue_dequeue(q);
        
       
        KASSERT((temp_thread->kt_state == KT_SLEEP) || (temp_thread->kt_state == KT_SLEEP_CANCELLABLE));
        dbg(DBG_PRINT,"(GRADING1A 4.a) \n" );
        
        sched_make_runnable(temp_thread);
        return temp_thread;
}
...
sched_switch(void)
{
        ...
        {
                 intr_disable();/*******************according to the google group*************************/
                 intr_setipl(IPL_LOW);
                 intr_wait();
                 intr_setipl(IPL_HIGH);                 
        }
