...
proc_create(char *name)
{
        ...
        /*Initialize process's thread list*/
        list_init(&(newProcess->p_threads));
        
        /*Initialize process's children list*/
        list_init(&(newProcess->p_children)); 
        
        /*Initialize to Parent process*/
	...
	while (i<NFILES)
	{
        ...
        if(curproc!=NULL && newProcess->p_pid!=PID_IDLE)
        {
        /*link on the list of children */
        list_insert_tail(&(curproc->p_children),&(newProcess->p_child_link));
        
		newMap = vmmap_create();
		newProcess->p_vmmap = newMap;
		newMap->vmm_proc = newProcess;
	...
	if (NULL != curproc && NULL != curproc->p_cwd)
	{
...
if(!list_empty(&curproc->p_children))
    {
     ...
     if(curproc->p_pproc->p_pid != PID_IDLE || curproc->p_pid==PID_INIT) {
...
proc_kill_all()
{
         ...
         if(!list_empty(list))
         {
             ...
             proc_t *p1;
             list_iterate_begin(list,p1,proc_t,p_list_link)
            {

                if(p1->p_pid != PID_IDLE)
                { 
                    if(p1->p_pproc->p_pid != PID_IDLE && p1!=curproc)
                   {
                      
                      proc_kill(p1,p1->p_status);
                   }
                }
             }
          list_iterate_end();
            ...
            while(1)
             {
                 list_iterate_begin(&curproc->p_children,childprocess,proc_t,p_child_link)
                {
                    ...
                    if(childprocess->p_state==PROC_DEAD)
                   {
					   
if(status != NULL) {
                           *status=childprocess->p_status;}
                            kthread_t *thread;
                            list_iterate_begin(&childprocess->p_threads,thread,kthread_t,kt_plink)
                           {
						       KASSERT(KT_EXITED == thread->kt_state);
                               dbg(DBG_PRINT,"GRADING1A 2.c) \n");
                              kthread_destroy(thread);
                           }list_iterate_end();
                           list_remove(&childprocess->p_child_link);


                           pt_destroy_pagedir(childprocess->p_pagedir);
                           slab_obj_free(proc_allocator, childprocess);
         ...
         if(pid>0)
         {
                 list_iterate_begin(&curproc->p_children,childprocess,proc_t,p_child_link)
                 {
                    ...
                    if(childprocess->p_pid==pid)
                   {

  while(childprocess->p_state != PROC_DEAD)
                           {
                             sched_sleep_on(&curproc->p_wait);
                           }
...
do_exit(int status)
{
   /*this is the same as call proc_kill on current process*/
