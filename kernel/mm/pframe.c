...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        pframe_t *temppage;
        do{
                temppage=pframe_get_resident(o,pagenum);
                if(temppage==NULL)
                {
                        if(pageoutd_needed())
                        {
                        ...
                        temppage=pframe_alloc(o,pagenum);
                        if(temppage==NULL)
                        {
                                return -ENOMEM;
                        }
                        else{
                                int res1=pframe_fill(temppage);
                                if(res1<0)
                                {       
                                        pframe_free(temppage);
                                        return res1;
                                }
                                *result=temppage;
                                return 0;
                        ...
                        if(pframe_is_busy(temppage))
                        {
                                /*resident && busy*/
                                sched_sleep_on(&temppage->pf_waitq);
                                continue;
                        }
                        else{
                                *result=temppage;
