...
sys_read(read_args_t *arg)
{
        ...
        read_args_t kaddr;
        int res1=copy_from_user(&kaddr,arg,sizeof(kaddr));/***********how to set the parameter***************/
        if(res1<0)
        {
        ...
        int res3=do_read(kaddr.fd,temp_buffer,kaddr.nbytes);/************return transfer data****************/
        if(res3<0)
        {
               curthr->kt_errno = -res3;/*remember to change it to be positive*/
               page_free(temp_buffer);
               return -1;
        }
        int res4= copy_to_user(arg->buf,temp_buffer,res3);/***************how to set the parameter***************/
        if(res4<0)
        {
               curthr->kt_errno = EFAULT;
               page_free(temp_buffer);
...
sys_write(write_args_t *arg)
{
        ...
        int res2=copy_from_user(temp_buffer,arg->buf,kaddr.nbytes);
        if(res2<0)
        {
               curthr->kt_errno = EFAULT;
               page_free(temp_buffer);
        ...
        int res3=do_write(kaddr.fd,temp_buffer,kaddr.nbytes);/************return transfer data****************/
        if(res3<0)
        {
               curthr->kt_errno = -res3;/*remember to change it to be positive*/
               page_free(temp_buffer);
...
sys_getdents(getdents_args_t *arg)
{
        ...
        getdents_args_t kaddr;
        int res1=copy_from_user(&kaddr,arg,sizeof(kaddr));
        if(res1<0)
        {
               curthr->kt_errno = -res1;
               return -1; 
        }
        size_t num=kaddr.count/sizeof(dirent_t);
        ...
        while(index<num)
        {        
                int res2=do_getdent(kaddr.fd, &tempdirp);
                if(res2<0)
                {
                        curthr->kt_errno=-res2;
                        return -1;
                }
                if(res2==0)
                {
                ...
                int res3=copy_to_user((void*)((uint32_t)kaddr.dirp+index*sizeof(dirent_t)),&tempdirp,sizeof(dirent_t));
                if(res3<0)
                {
                        curthr->kt_errno=-res3;
                        return -1;
                }
                amount+=sizeof(dirent_t);
                index=index+1;
        }
        return amount;
}
