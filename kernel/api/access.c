...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        vmarea_t *target_vmarea=vmmap_lookup(map, res_vfn);
        if(target_vmarea==NULL)
        {
                return 0;
        }

        if(target_vmarea->vma_prot & perm)
        {
                return 1;
        }
        return 0;
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        vmmap_t *map=p->p_vmmap;
        uint32_t res_vfn1=ADDR_TO_PN(avaddr);
        uint32_t res_vfn2=ADDR_TO_PN((void*)((uintptr_t)avaddr+len));
        while(res_vfn1<=res_vfn2)
        {
                vmarea_t *target_vmarea=vmmap_lookup(map, res_vfn1);
                if(target_vmarea==NULL)
                {
                        return 0;
                }
